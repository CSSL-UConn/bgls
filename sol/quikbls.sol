...
contract QuickBLS{
    ...
    function verifySignature (uint256[2] memory signature,uint256[4] memory pubkey, bytes memory message) public  returns (bool) {
   ...
   function ecCheckPairing(uint256[12] memory input) private returns (bool) {
    ...
    return result[0] == 1;
      ...
      function VerifyMsgHash(uint256[2] memory hash, bytes memory message)public view returns (bool) {
        uint256[2] memory hm = htp(message);
